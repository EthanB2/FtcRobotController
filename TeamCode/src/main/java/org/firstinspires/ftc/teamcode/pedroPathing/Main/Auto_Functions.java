...
import org.firstinspires.ftc.teamcode.pedroPathing.examples.ExampleBucketAuto;
...
public class Auto_Functions {
    ...
    private static final double YAW_MAX_AUTO        = 1.0;     // "default" Maximum Yaw power limit during autonomous
...
//    private CRServo intake = null;
    ...
    public void initialize(boolean showTelemetry)
    {
        ...
        tiltMotor = setupDriveMotor("tilt",DcMotor.Direction.FORWARD);
        extendMotor = setupDriveMotor("extend",DcMotor.Direction.REVERSE);
...
//2.375 in to the right, 6.125 in forward
        SparkFunOTOS.Pose2D offset = new SparkFunOTOS.Pose2D(-2.368, -6.65, 0);
        myOtos.setOffset(offset);

        myOtos.setLinearScalar(0.9690);
        myOtos.setAngularScalar(1.0028);
...
//        rawStrafeOdometer = strafeEncoder.getCurrentPosition() * (INVERT_STRAFE_ODOMETRY ? -1 : 1);
        ...
        driveDistance = -pos.y;
//                (rawDriveOdometer - driveOdometerOffset) * ODOM_INCHES_PER_COUNT;
        strafeDistance =  pos.x;
//                (rawStrafeOdometer - strafeOdometerOffset) * ODOM_INCHES_PER_COUNT;
...
//        if (showTelemetry) {
//            myOpMode.telemetry.addData("Odom Ax:Lat", "%6d %6d", pos.y, pos.x);
...
//    public void driveANDtilt(double distanceInches, double tilt, double power, double holdTime) {
//        resetOdometry();
//
...
//            tiltArm(tilt, power);
//            // Time to exit?
//            if (driveController.inPosition() && yawController.inPosition() && Math.abs(tiltMotor.getCurrentPosition()-tiltValue)<5) {
...
//
//    public void intakeStop() {intake.setPower(0); }
